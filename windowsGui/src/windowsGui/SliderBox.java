...
//				save();
	...
	public void drawToolTip(PGraphics canvas){
		...
		if(tooltip!=null&&tooltipPos2(mouse)&&tooltip.visible&&!t2down){
			...
			if(!theme.border)canvas.noStroke();
			canvas.fill(theme.menufillcol,theme.menutransparency);
			if(!theme.fill)canvas.noFill();
	...
	public void drawToolTip(){
		if(!tdown&&tooltip!=null&&tooltipPos()&&!t2down&&tooltip.visible){
			...
			if(!theme.border)applet.noStroke();
			applet.fill(theme.menufillcol,theme.menutransparency);
			if(!theme.fill)applet.noFill();
	...
	public void setPie(int a,float b, float c){
		getSlider(a).parent = this.menu;
		if(a==menu.sindex)getSlider(a).setPie(b,c);
	};
	
	public void setPieInt(int a,int b, int c){
//		if(applet.mousePressed)applet.println("sliderb setpieint",a,menu.sindex);

		getSlider(a).parent = this.menu;
		getSlider(a).INT = true;
		if(a==menu.sindex)getSlider(a).setPieInt(b,c);
	};
...
//		else Bms.output.write("plain");
	...
	public void setPie(int a, float b,float c,Object o, String variable,String n){
		if(a==menu.sindex){
			getSlider(a).parent = this.menu;
			getSlider(a).parentObject = o;
			getSlider(a).parentVar = variable;
	...
	public void setPieInt(int a, int b,int c,Object o, String variable,String n){
		if(a==menu.sindex){
			getSlider(a).INT = true;
			getSlider(a).parent = this.menu;
			getSlider(a).parentObject = o;
	...
	public void add(String label) {
		Slider s = new Slider(x,y+sliders.size()*sliders.get(0).h,w,sliders.get(0).h,label,Bms);
		if(menu.pieSquare)s.setPieSquare();
		if(menu.classicBar)s.setClassicBar();
		if(menu.classicSquare)s.setClassicSquare();
		if(menu.classicRadio)s.setClassicRadio();
		sliders.add(s);
		menu.sliders.add(s);
	};
